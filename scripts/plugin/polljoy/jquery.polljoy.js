...
(function (polljoyFactory) {
    ...
    var isAutoShowQuestion=false;
    var autoShowQuestionInverval=20000;
    var pollId;
    var question_timer;
    var appId; 
    ...
    var methods = {
        init: function init(configuration)
        {
            ...
            if (typeof configuration.isAutoShowQuestion !== 'undefined')
                isAutoShowQuestion = configuration.isAutoShowQuestion;

            if (typeof configuration.pollId !== 'undefined')
                pollId = configuration.pollId;

            if (typeof configuration.appId !== 'undefined')
            ...
            var registerSession = jQuery.ajax({
                ...
                if (response.status !== 0)  // AM, 20150508
                {
                    ...
                    success: function (sgResponse) {
                        ...
                        {
                             for (var index = 0; index <= sgResponse.polls.length -1; index++) {

                             var poll = sgResponse.polls[index];
                            
                            if(poll.PollRequest.pollId==pollId)
                            {
                                 var _poll = [];
                                 _poll.push(poll);
                                 polls=_poll;
        ...
        {
            console.log(autoShowQuestionInverval);
           if(isAutoShowQuestion)
           {
                // Need to clear the timer 
                clearInterval(question_timer);
                question_timer = setInterval(function() {
                        console.log("Call to show the next question automatically.");
                        methods.drawNextQuestion1.apply(thisObject);

                },autoShowQuestionInverval);
            }

           methods.drawNextQuestion1.apply(thisObject);
        },

        drawNextQuestion1: function()
        {
             
            console.log("question shown"+current++);
        ...
        bindEvents: function()
        {
            ...
            timer = setInterval(function() {
              console.log('how many time, user will take to anser the question');
               methods.centerThePoll();
            }, 1000);
        },
    ...
    $.fn.polljoy = function(method,retval) {
